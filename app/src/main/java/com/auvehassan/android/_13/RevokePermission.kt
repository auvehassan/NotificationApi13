...
class RevokePermission : AppCompatActivity() {
    ...
    private fun initializeUI() {
        ...
        buttonRevokeCameraPermission.setOnClickListener {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                revokeSelfPermissionOnKill(CAMERA)
                showRevokeSuccessSnackBar()
            }
        }
        buttonRevokeCallPermission.setOnClickListener {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                revokeSelfPermissionOnKill(CALL_PHONE)
                showRevokeSuccessSnackBar()
            }
        }
    ...
    private fun refreshUI() {
        val isCameraPermissionGranted = isPermissionGranted(CAMERA)
        val isCallPermissionGranted = isPermissionGranted(CALL_PHONE)
